...
const CreatePage = ({ onSelectOption }: Props) => {
  const { setPage, prompts } = usePromptStore();
  ...
  return (
    ...
    >
      <motion.div variants={itemsVariants} className="text-center space-y-2">
      ...
      >
        {CreatePageCard.map((option) => (
          ...
          >
            <motion.div
              className="w-full h-full p-4 flex flex-col gap-y-6 items-start bg-white dark:bg-black rounded-xl"
            ...
            >
              <div className="flex flex-col items-start w-full gap-y-3">
                ...
                <p className="text-primary opacity-50 text-sm font-normal">
                  {option.description}
                </p>
              </div>
              <motion.div
